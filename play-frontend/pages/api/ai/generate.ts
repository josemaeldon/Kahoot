...
function parseGeneratedPlay(
  value: string,
  categoryName: string
): GeneratedPlay {
  const parsed = JSON.parse(value) as Partial<GeneratedPlay>;
  if (
    ...
    throw new Error("A IA retornou um Play! incompleto.");
  }
...
export default async function handler(
  ...
  if (Date.now() - lastRequest < 15_000) {
    return res.status(429).json({
      error: true,
      errorDescription: "Aguarde alguns segundos antes de gerar outro Play!",
    });
  }
  ...
  try {
    ...
    if (!category) {
      return res.status(400).json({
        error: true,
        errorDescription: "Selecione uma categoria válida para o Play!",
      });
    }
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      const upstream = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          ...
          safety_identifier: crypto
            ...
            .digest("hex"),
          instructions: [
            "Você cria Plays! em português do Brasil para uso educacional.",
            ...
            `Crie o título no padrão dos Plays! públicos: "${category.name} — [tema curto e específico]".`,
            ...
            .filter(Boolean)
            .join("\n"),
          input: `Crie um Play! da categoria "${category.name}" com base neste pedido do usuário:\n\n${prompt}`,
        ...
        const message =
          upstream.status === 401
            ...
            : upstream.status === 429
              ...
              : "A OpenAI não conseguiu gerar o Play! agora.";
      ...
      const game = parseGeneratedPlay(text, category.name);
    ...
    console.error("Falha ao gerar Play! com IA", error);
    const timedOut =
