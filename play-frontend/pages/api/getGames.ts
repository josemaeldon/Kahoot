...
interface Success {
  error: false;
  games: db.PlaySummary[];
  folders: db.PlayFolder[];
  organization: {
    totalCount: number;
    unfiledCount: number;
  };
  pagination: Pagination;
  publicAuthors: db.PlayAuthor[];
}
...
export default async function handler(
  ...
  try {
    ...
    console.error("Falha ao listar Plays!", error);
    return res.status(500).json({
      error: true,
      errorDescription: "Não foi possível carregar os Plays!",
    });
  }
}
