...
export default async function handler(
  ...
  if (!UUID_PATTERN.test(request.gameId)) {
    return res
      .status(400)
      .json({ error: true, errorDescription: "Play! inválido." });
  }
  ...
  try {
    if (request.action === "move") {
      ...
      if (!moved) {
        return res.status(404).json({
          error: true,
          errorDescription: "Play! ou pasta não encontrado.",
        });
      }
    ...
    console.error("Falha ao organizar Play!", error);
    return res.status(500).json({
      error: true,
      errorDescription: "Não foi possível atualizar o Play!",
    });
  }
}
