...
export class PlayCsvError extends Error {}
...
export function parsePlayCsv(rawSource: string): db.PlayGame {
  let source = rawSource.replace(/^\uFEFF/, "").trim();
  if (!source) {
    throw new PlayCsvError("O arquivo CSV está vazio.");
  }
  ...
  rows.slice(1).forEach((row, rowIndex) => {
    ...
    const rowTitle = read(row, "titulo_play");
    if (rowTitle) {
  ...
  if (!title) {
    throw new PlayCsvError(
      "Informe o titulo_play na primeira linha de pergunta."
    );
  }
  if (title.length > 120) {
    throw new PlayCsvError("O título deve ter no máximo 120 caracteres.");
  }
  if (questions.length > 100) {
    throw new PlayCsvError("O arquivo pode conter no máximo 100 perguntas.");
  }
